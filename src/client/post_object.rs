...
impl PostObjectFluentBuilder {
    ...
    pub fn x_oss_server_side_encryption_key_id(
        ...
        self.inner.x_oss_server_side_encryption_key_id =
            Some(x_oss_server_side_encryption_key_id.into());
        self
    }
    ...
    pub fn x_oss_server_side_data_encryption(
        ...
        self.inner.x_oss_server_side_data_encryption =
            Some(x_oss_server_side_data_encryption.into());
        self
    }
    ...
    pub async fn send(self) -> Result<PostObjectOutput, OSSError> {
        ...
        let (policy, signature) = if let Some(ref p) = self.inner.policy {
            ...
            let sig = self
                ...
                .signature
                .clone()
                .unwrap_or_else(|| calculate_signature(access_key_secret, p));
            (p.clone(), sig)
        } else {
            // 自动生成 policy 和签名
        ...
        form_data.extend_from_slice(format!("--{}\r\n", boundary).as_bytes());
        form_data
            .extend_from_slice(b"Content-Disposition: form-data; name=\"OSSAccessKeyId\"\r\n\r\n");
        ...
        if let Some(ref x_oss_security_token) = self.inner.x_oss_security_token {
            ...
            form_data.extend_from_slice(
                b"Content-Disposition: form-data; name=\"x-oss-security-token\"\r\n\r\n",
            );
        ...
        if let Some(ref content_disposition) = self.inner.content_disposition {
            form_data.extend_from_slice(format!("--{}\r\n", boundary).as_bytes());
            form_data.extend_from_slice(
                b"Content-Disposition: form-data; name=\"Content-Disposition\"\r\n\r\n",
            );
        ...
        if let Some(ref cache_control) = self.inner.cache_control {
            form_data.extend_from_slice(format!("--{}\r\n", boundary).as_bytes());
            form_data.extend_from_slice(
                b"Content-Disposition: form-data; name=\"Cache-Control\"\r\n\r\n",
            );
        ...
        if let Some(ref content_encoding) = self.inner.content_encoding {
            form_data.extend_from_slice(format!("--{}\r\n", boundary).as_bytes());
            form_data.extend_from_slice(
                b"Content-Disposition: form-data; name=\"Content-Encoding\"\r\n\r\n",
            );
        ...
        if let Some(ref expires) = self.inner.expires {
            form_data.extend_from_slice(format!("--{}\r\n", boundary).as_bytes());
            form_data
                .extend_from_slice(b"Content-Disposition: form-data; name=\"Expires\"\r\n\r\n");
        ...
        if let Some(ref x_oss_object_acl) = self.inner.x_oss_object_acl {
            form_data.extend_from_slice(format!("--{}\r\n", boundary).as_bytes());
            form_data.extend_from_slice(
                b"Content-Disposition: form-data; name=\"x-oss-object-acl\"\r\n\r\n",
            );
        ...
        if let Some(ref x_oss_storage_class) = self.inner.x_oss_storage_class {
            form_data.extend_from_slice(format!("--{}\r\n", boundary).as_bytes());
            form_data.extend_from_slice(
                b"Content-Disposition: form-data; name=\"x-oss-storage-class\"\r\n\r\n",
            );
        ...
        if let Some(ref x_oss_server_side_encryption) = self.inner.x_oss_server_side_encryption {
            form_data.extend_from_slice(format!("--{}\r\n", boundary).as_bytes());
            form_data.extend_from_slice(
                b"Content-Disposition: form-data; name=\"x-oss-server-side-encryption\"\r\n\r\n",
            );
            form_data.extend_from_slice(x_oss_server_side_encryption.as_bytes());
            form_data.extend_from_slice(b"\r\n");
        }

        if let Some(ref x_oss_server_side_encryption_key_id) =
            self.inner.x_oss_server_side_encryption_key_id
        {
            form_data.extend_from_slice(format!("--{}\r\n", boundary).as_bytes());
        ...
        if let Some(ref x_oss_server_side_data_encryption) =
            self.inner.x_oss_server_side_data_encryption
        {
            form_data.extend_from_slice(format!("--{}\r\n", boundary).as_bytes());
        ...
        if let Some(x_oss_forbid_overwrite) = self.inner.x_oss_forbid_overwrite {
            form_data.extend_from_slice(format!("--{}\r\n", boundary).as_bytes());
            form_data.extend_from_slice(
                b"Content-Disposition: form-data; name=\"x-oss-forbid-overwrite\"\r\n\r\n",
            );
            form_data.extend_from_slice(if x_oss_forbid_overwrite {
                b"true"
            } else {
                b"false"
            });
        ...
        for (meta_key, meta_value) in &self.inner.user_metadata {
            ...
            form_data.extend_from_slice(
                format!(
                    "Content-Disposition: form-data; name=\"x-oss-meta-{}\"\r\n\r\n",
                    meta_key
                )
                .as_bytes(),
            );
...
mod tests {
    ...
    fn test_builder_with_options() {
        ...
        assert_eq!(
            builder.inner.success_action_status,
            Some(SuccessActionStatus::Status200)
        );
        assert_eq!(
            builder.inner.user_metadata.get("uuid"),
