...
use crate::{
    auth::Auth, errors::status_to_response, model::{object::ListBucketResult, Empty}, oss::{ObjectMeta, RequestType}, prelude::OSS
};
...
use bytes::Bytes;
use reqwest::{header::{HeaderMap, HeaderValue, DATE}, StatusCode};

pub(crate) trait PrivateObjectAPI {
...
impl<'a> ObjectAPI for OSS<'a> {
        ...
        S: AsRef<str> + Send,
    {
        ...
        resources.insert("prefix".to_string(), prefix.map(|s| s.as_ref().to_string()));
        resources.insert("max-keys".to_string(), Some("1000".to_string()));
        let headers = HashMap::new();
        let (host, headers) =
            self.build_request(RequestType::Get, String::new(), Some(headers), resources)?;
