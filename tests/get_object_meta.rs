...
async fn test_get_object_meta() {
    ...
    assert!(output.content_length.is_some(), "Content-Length should be present");
    assert_eq!(output.content_length.unwrap(), test_content.len() as u64);
    assert!(output.last_modified.is_some(), "Last-Modified should be present");
...
async fn test_get_object_meta_nonexistent() {
    ...
    // 尝试获取不存在的对象元数据
    let result = client.get_object_meta()
        .bucket(&std::env::var("OSS_BUCKET").unwrap())
