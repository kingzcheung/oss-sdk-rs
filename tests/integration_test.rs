...
async fn test_object_lifecycle() -> Result<(), OSSError> {
    ...
    let head_output = oss.head_object()
        .bucket(&bucket)
        .key(object_name)
        .send()
        .await?;
    ...
    let delete_output = oss.delete_object()
        .bucket(&bucket)
        .key(object_name)
        .send()
        .await?;
...
async fn test_get_object_with_range() -> Result<(), OSSError> {
    ...
    println!("Range request content: {:?}", String::from_utf8_lossy(&partial_content));
